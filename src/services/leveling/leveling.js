...
export function calculateTotalXp(level, currentXp = 0) {
    if (!Number.isInteger(level) || level < 0) {
        return Math.max(0, Number(currentXp) || 0);
    }
...
export async function getLevelingConfig(
    ...
    try {
        ...
        return {
            ...
            xpRange: {
                ...defaults.xpRange,
                ...(guildConfig?.leveling?.xpRange || {})
            },

            ignoredChannels:
                Array.isArray(
                    guildConfig?.leveling?.ignoredChannels
                )
                    ? guildConfig.leveling.ignoredChannels
                    : [],
            ...
            ignoredRoles:
                Array.isArray(
                    guildConfig?.leveling?.ignoredRoles
                )
                    ? guildConfig.leveling.ignoredRoles
                    : [],
            ...
            blacklistedUsers:
                Array.isArray(
                    guildConfig?.leveling?.blacklistedUsers
                )
                    ? guildConfig.leveling.blacklistedUsers
                    : [],

            roleRewards:
                guildConfig?.leveling?.roleRewards || {}
        };
...
export async function saveLevelingConfig(
    ...
    try {
        ...
        if (
            config.xpCooldown !== undefined
        ) {
            const cooldown =
                Number(config.xpCooldown);
        ...
        if (config.xpRange) {
            const min =
                Number(config.xpRange.min);

            const max =
                Number(config.xpRange.max);
