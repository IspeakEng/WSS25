...
const EVENT_TYPES = {
  ...
  MEMBER_PROFILE_CHANGE: 'member.profilechange',
  MEMBER_PRESENCE_CHANGE: 'member.presencechange',
...
const EVENT_COLORS = {
  ...
  'member.profilechange': 0x9b59b6,
  'member.presencechange': 0x5865F2,
...
const EVENT_ICONS = {
  ...
  'member.leave': '👋',
  'member.namechange': '🏷️',
  'member.profilechange': '👤',
  'member.presencechange': '🟢',
...
function getLogChannelForEvent(
  ...
  const category = eventType?.split('.')[0];
  const destination =
    CATEGORY_DESTINATION[category] || 'audit';
  ...
  try {
    ...
    types.forEach((type) => {
      if (type.endsWith('.*')) {
        ...
        const matchingTypes =
          Object.values(EVENT_TYPES).filter(
            (eventType) =>
